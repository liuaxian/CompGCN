...
class CompGCNConvBasis(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, edge_norm=None, rel_embed=None, edge_weight=None):
        ...
        if edge_weight is not None:
            in_weight = edge_weight[:num_edges]
            out_weight = edge_weight[num_edges:]
        else:
            in_weight = None
            out_weight = None

        # MODIFIED: propagate 调用增加 edge_weight 参数
        in_res		= self.propagate('add', self.in_index,   x=x, edge_type=self.in_type,   rel_embed=rel_embed, edge_norm=self.in_norm,  edge_weight=in_weight, mode='in')
        loop_res	= self.propagate('add', self.loop_index, x=x, edge_type=self.loop_type, rel_embed=rel_embed, edge_norm=None, 	      edge_weight=None,      mode='loop')
        out_res		= self.propagate('add', self.out_index,  x=x, edge_type=self.out_type,  rel_embed=rel_embed, edge_norm=self.out_norm, edge_weight=out_weight, mode='out')
        out		= self.drop(in_res)*(1/3) + self.drop(out_res)*(1/3) + loop_res*(1/3)

        if self.p.bias: out = out + self.bias
        # MODIFIED: 移除有问题的 b_norm 条件，直接应用 BN，与标准版一致
        out = self.bn(out)
    ...
    # MODIFIED: message 增加 edge_weight 参数，并乘以权重
