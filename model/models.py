...
class CompGCNBase(BaseModel):
    # MODIFIED: 增加 edge_weight 参数，默认 None
    def __init__(self, edge_index, edge_type, num_rel, params=None, edge_weight=None):
        super(CompGCNBase, self).__init__(params)

        self.edge_index        = edge_index
        self.edge_type        = edge_type
        self.edge_weight    = edge_weight    # 新增，保存边权重
    ...
    def forward_base(self, sub, rel, drop1, drop2):
        ...
        # MODIFIED: 调用 conv1 时传入 edge_weight
        x, r    = self.conv1(self.init_embed, self.edge_index, self.edge_type, rel_embed=r, edge_weight=self.edge_weight)
        x    = drop1(x)
        # MODIFIED: 调用 conv2 时传入 edge_weight（如果存在）
        x, r    = self.conv2(x, self.edge_index, self.edge_type, rel_embed=r, edge_weight=self.edge_weight) if self.p.gcn_layer == 2 else (x, r)
...
class CompGCN_TransE(CompGCNBase):
    # MODIFIED: 增加 edge_weight 参数并传递给父类
    def __init__(self, edge_index, edge_type, params=None, edge_weight=None):
        super(self.__class__, self).__init__(edge_index, edge_type, params.num_rel, params, edge_weight=edge_weight)
